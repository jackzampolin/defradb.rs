...
| `630f4f30` | #4763 | fix: Prevent txn-actions from executing concurrently | **already-ported** | Rust commit `b6255b00` in PR #951 ("fix: serialize actions per transaction"). Per-txn `action_lock: Arc<async_lock::Mutex<()>>` at `crates/db/src/txn/context.rs:32`, acquired in `crates/query/src/runner/executor.rs:302-306` and `crates/db/src/txn/registry/`. Test: `execute_in_txn_serializes_concurrent_actions_on_same_handle`. |
