...
#[tokio::test]
async fn test_redb_chunked_scan_crosses_several_chunks() {
    assert_full_scan_matches(1000).await;
}

#[tokio::test]
async fn test_redb_chunked_scan_exact_chunk_multiple() {
    assert_full_scan_matches(256).await;
}

#[tokio::test]
async fn test_redb_chunked_scan_one_over_chunk() {
    assert_full_scan_matches(257).await;
}

#[tokio::test]
async fn test_redb_chunked_scan_single_key() {
    assert_full_scan_matches(1).await;
}

#[tokio::test]
async fn test_redb_chunked_scan_empty_store() {
...
async fn seeded_store(count: u32) -> (TempDir, RedbStore, Vec<Vec<u8>>) {
    let temp_dir = TempDir::new().unwrap();
    let store = RedbStore::open(temp_dir.path().join("test.redb")).unwrap();
    let expected: Vec<Vec<u8>> = (0..count)
        .map(|i| format!("key_{:05}", i).into_bytes())
...
#[tokio::test]
async fn test_redb_chunked_seek_backward_across_discarded_windows() {
...
#[tokio::test]
async fn test_redb_chunked_seek_forward_beyond_loaded_window() {
    let (_temp_dir, store, expected) = seeded_store(1000).await;
