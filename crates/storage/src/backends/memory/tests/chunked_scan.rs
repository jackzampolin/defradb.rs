...
#[tokio::test]
async fn test_memory_chunked_scan_crosses_several_chunks() {
    assert_full_scan_matches(1000).await;
}

#[tokio::test]
async fn test_memory_chunked_scan_exact_chunk_multiple() {
    assert_full_scan_matches(256).await;
}

#[tokio::test]
async fn test_memory_chunked_scan_one_over_chunk() {
    assert_full_scan_matches(257).await;
}

#[tokio::test]
async fn test_memory_chunked_scan_single_key() {
    assert_full_scan_matches(1).await;
}

#[tokio::test]
async fn test_memory_chunked_scan_empty_store() {
    assert_full_scan_matches(0).await;
}

/// Build a store with `count` zero-padded keys and return the expected order.
async fn seeded_store(count: u32) -> (MemoryStore, Vec<Vec<u8>>) {
    let store = MemoryStore::new();
    let expected: Vec<Vec<u8>> = (0..count)
        .map(|i| format!("key_{:05}", i).into_bytes())
        .collect();
...
/// A backward seek after the snapshot has already walked past several
...
#[tokio::test]
async fn test_memory_chunked_seek_backward_across_discarded_windows() {
    let (store, expected) = seeded_store(1000).await;
...
#[tokio::test]
async fn test_memory_chunked_seek_forward_beyond_loaded_window() {
    let (store, expected) = seeded_store(1000).await;
