//! Collection version sync via transport-generic block fetching.
//!
//! Transport-agnostic equivalent of `version_syncer.rs`. Uses `P2PTransport`
//! methods (sync_blocks) instead of `P2PHostHandle` (bitswap_sync).
...
/// Fetch a single block via transport, polling until available.
async fn fetch_block<B: Blockstore, T: P2PTransport>(
    ...
    transport
        ...
        .await
        .map_err(|e| format!("block sync for {}: {}", target_cid, e))?;
...
/// Fetch lens IPLD blocks via transport and register the WASM module.
    ...
    async fn sync_versions(
        ...
        for version_id_str in &version_ids {
            ...
            if let Err(e) = self
                ...
                .await
            {
                tracing::warn!(cid = %version_cid, error = %e, "block sync failed");
                continue;
            }
            ...
            while let Some(link_cid) = fetch_queue.pop_front() {
                ...
                if !already_present {
                    if let Err(e) = self
                        ...
                        .await
                    {
                        tracing::warn!(cid = %link_cid, error = %e, "block sync failed for link");
                        continue;
                    }
            ...
            match defra_core::Block::from_dag_cbor(&block_data) {
                Ok(block) => {
                    if let defra_core::CrdtDelta::CollectionDefinition(ref payload) = block.delta {
                        if let Some(ref transform_cid) = payload.query_transform {
                            tracing::info!(transform_cid = %transform_cid, "syncing lens for view");
                ...
                Err(e) => {
                    tracing::warn!(cid = %version_cid, error = %e, "failed to decode version block");
                }
            }
        }
