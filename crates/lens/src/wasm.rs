...
mod tests {
    ...
    #[tokio::test]
    async fn test_wasm_set_default_transform() {
        use futures::StreamExt;

        let wasm_path = "/Users/johnzampolin/go/src/github.com/sourcenetwork/defradb/tests/lenses/rust_wasm32_set_default/target/wasm32-unknown-unknown/debug/rust_wasm32_set_default.wasm";
        ...
        let module = LensModule::from_path(wasm_path).with_arguments(serde_json::json!({
            "dst": "verified",
            "value": true
        }));
        let config = LensConfig::new("v1", "v2", module);

        let transform_id = store.add(config).await.unwrap();
        eprintln!("Transform ID: {}", transform_id);
        assert!(store.has_transform(&transform_id));

        // Create a test document
        let mut doc = LensDoc::new();
        doc.insert("name".to_string(), serde_json::json!("Alice"));
        doc.insert("_docID".to_string(), serde_json::json!("bafyrei123"));

        eprintln!("Input doc: {:?}", doc);

        // Execute forward transform
        let input_stream: crate::store::LensDocStream =
            Box::pin(futures::stream::once(async move { doc }));
        let mut output_stream = store.transform(&transform_id, input_stream).unwrap();

        let result = output_stream.next().await;
        eprintln!("Transform result: {:?}", result);

        let output_doc = result.unwrap().unwrap();
        eprintln!("Output doc: {:?}", output_doc);

        // The set_default lens should have added verified: true
        assert_eq!(
            output_doc.get("name").unwrap(),
            &serde_json::json!("Alice"),
            "name field should be preserved"
        );
        assert_eq!(
            output_doc.get("verified").unwrap(),
            &serde_json::json!(true),
            "verified field should be set to true by set_default lens"
        );
    }

    #[tokio::test]
    async fn test_wasm_set_default_via_lens_pipeline() {
        use crate::history::TargetedHistoryLink;
        use crate::pipeline::Lens;
        use std::collections::HashMap;
        use std::sync::Arc;
        ...
        let store = Arc::new(WasmTransformStore::new().unwrap());
        ...
        // Build targeted history: v1 -> v2 with transform on v2
        let mut history: HashMap<String, TargetedHistoryLink> = HashMap::new();
        history.insert(
            "v1".to_string(),
        ...
        let mut lens = Lens::new(
            store as Arc<dyn crate::store::TransformStore>,
            "v2",
            history,
        );
        ...
        // Get migrated document (should be at v2 with verified: true)
        let result = lens.next().await;
        eprintln!("Pipeline result: {:?}", result);

        let migrated = result.unwrap().unwrap();
        eprintln!("Migrated doc: {:?}", migrated);

        assert_eq!(migrated.get("name").unwrap(), &serde_json::json!("Alice"));
        assert_eq!(
            migrated.get("verified").unwrap(),
            &serde_json::json!(true),
            "verified field should be set to true after migration through pipeline"
        );
    }
}
