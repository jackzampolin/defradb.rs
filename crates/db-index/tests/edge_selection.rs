...
use db_index::vector::core::{dot, Metric};
use db_index::vector::engine::ann::{Candidate, EdgeSelector};
use db_index::vector::engine::select::{Angular, Heuristic, DEFAULT_ANGLE_DEGREES};
...
fn both_selectors_respect_the_cap() {
    let base = [0.0f32, 0.0];
    let mut corpus = common::Corpus::new(0xED6E);
...
fn angular_pruning_holds_its_angle() {
    let base = [0.0f32, 0.0];
    let mut corpus = common::Corpus::new(0x5563);
...
fn a_wider_angle_keeps_fewer_edges() {
    let base = [0.0f32, 0.0];
    let mut corpus = common::Corpus::new(0xA9613);
...
fn the_expanded_form_matches_the_naive_one() {
    let mut corpus = common::Corpus::new(0x9A17E);
...
fn the_nearest_candidate_is_always_kept() {
    let base = [0.0f32, 0.0];
    let mut corpus = common::Corpus::new(0x11157);
