...
fn graph(seed: u64) -> Hnsw<MemoryNodeStore> {
    Hnsw::new(
        MemoryNodeStore::new(),
        Metric::Cosine,
...
async fn build(vectors: &[Vec<f32>], seed: u64) -> Hnsw<MemoryNodeStore> {
...
fn scored(vectors: &[Vec<f32>], query: &[f32], k: usize) -> Vec<NodeId> {
...
async fn flat(vectors: &[Vec<f32>]) -> Flat<MemoryNodeStore> {
