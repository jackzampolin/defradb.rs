...
use sqlparser::ast::{
    AlterTableOperation, AssignmentTarget, BinaryOperator, ColumnDef, ColumnOption, DataType, Expr,
    FromTable, ObjectName, OnConflictAction, OnInsert, OrderByExpr, OrderByKind, ReferentialAction,
    SelectItem, SetExpr, Statement, TableConstraint, TableFactor, TableObject, Value,
};
...
pub fn sql_to_graphql_typed(
    ...
    match &statements[0] {
        ...
        Statement::CreateIndex(ci) => translate_create_index(ci),
        Statement::AlterTable {
            name, operations, ..
        } => translate_alter_table(name, operations),
...
fn translate_create_index(ci: &sqlparser::ast::CreateIndex) -> Result<SqlStatement, PgCompatError> {
...
/// Extract just the table name from a potentially schema-qualified ObjectName.
