//! Dense retrieval v1 primitives for embedded DefraDB nodes.
//!
...
impl EmbeddedNode {
    ...
    ///
    /// This uses DefraDB's dense-search v1 contract:
    /// - POST to `.../embeddings`
    /// - body `{ "model": "...", "input": "..." }`
    /// - parse `data[0].embedding` as a numeric vector
    ///
    /// Any model-specific preprocessing should already be handled by the remote
    /// embedding service.
    ...
    pub async fn hybrid_search_dense(
        ...
        hybrid_search_dense(self, request).await
    }
}

async fn hybrid_search_dense(
    node: &EmbeddedNode,
    request: &DenseHybridSearchRequest,
) -> Result<DenseHybridSearchResponse> {
    ...
    let embedding_model = request
        ...
        .clone()
        .or_else(|| {
            let default_model = node.embedding_config().model.trim();
    ...
    let bm25_data = require_success(node.execute(&bm25_query).await, "dense-search bm25")?;
    let dense_data = require_success(node.execute(&dense_query).await, "dense-search similarity")?;
...
pub(crate) async fn embed_text_with_config(
    ...
    let mut request = embedding_client().post(endpoint).json(&json!({
...
pub(crate) fn require_success(response: crate::QueryResponse, context: &str) -> Result<JsonValue> {
