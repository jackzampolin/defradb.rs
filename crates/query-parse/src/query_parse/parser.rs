...
use super::aggregates::{
    grouped_relation_selects, parse_aggregate_field, parse_group_by_value,
    parse_top_level_aggregate,
};
use super::explain::{
        ...
        match selection {
            Selection::Field(field) => {
                ...
                if let Some(agg_type) = AggregateType::parse(&field_name) {
                    let (mut aggregate, target_groups) =
                        parse_aggregate_field(field, agg_type, variables)?;
                    ...
                    let grouped_selects = grouped_relation_selects(&mut aggregate, target_groups);
