...
/// ACP-backed permission checking for `PermissionFilterNode`.
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
pub trait DocPermissionChecker: Send + Sync {
...
struct AcpPermissionChecker {
    acp: Arc<dyn DocumentACP>,
    identity: Identity,
}

impl AcpPermissionChecker {
    fn new(acp: Arc<dyn DocumentACP>, identity: Identity) -> Self {
        Self { acp, identity }
    }
}
...
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl DocPermissionChecker for AcpPermissionChecker {
    async fn has_read_permission(
        &self,
        policy_id: &str,
        resource_name: &str,
...
pub struct PermissionFilterNode {
    ...
    /// Permission checker for document access.
    checker: Arc<dyn DocPermissionChecker>,
...
impl PermissionFilterNode {
    ...
    pub fn new(
        ...
        Self::with_checker(
            source,
            Arc::new(AcpPermissionChecker::new(acp, identity)),
            policy_id,
            resource_name,
        )
    }
