...
const HKDF_AES_KEY: [u8; 32] = [
    0xea, 0x1d, 0x8a, 0x20, 0xf4, 0x76, 0xd1, 0xe1, 0xec, 0x95, 0x2c, 0xa4, 0x27, 0x08, 0xb8, 0xf7,
    0x16, 0x1c, 0xe7, 0xc8, 0x1e, 0xad, 0xf9, 0x7e, 0x52, 0x0e, 0x2b, 0x40, 0x33, 0x3d, 0xec, 0xd5,
];
const HKDF_HMAC_KEY: [u8; 32] = [
    0x66, 0x98, 0xbc, 0x97, 0xa8, 0xce, 0x75, 0x06, 0x84, 0x9b, 0xe3, 0x20, 0x17, 0x5a, 0x48, 0x32,
    0xc5, 0xce, 0x24, 0x62, 0xe9, 0xc3, 0x0c, 0xd4, 0x30, 0x0b, 0x04, 0xa2, 0x8d, 0x75, 0xbf, 0xa5,
];
...
const ECIES_CIPHERTEXT: &[u8] = &[
    0x85, 0x20, 0xf0, 0x09, 0x89, 0x30, 0xa7, 0x54, 0x74, 0x8b, 0x7d, 0xdc, 0xb4, 0x3e, 0xf7, 0x5a,
    0x0d, 0xbf, 0x3a, 0x0d, 0x26, 0x38, 0x1a, 0xf4, 0xeb, 0xa4, 0xa9, 0x8e, 0xaa, 0x9b, 0x4e, 0x6a,
    0x64, 0x65, 0x74, 0x65, 0x72, 0x6d, 0x69, 0x6e, 0x69, 0x73, 0x74, 0x69, 0x06, 0x37, 0x0d, 0xcb,
    0x50, 0x3d, 0x96, 0x30, 0x77, 0xa7, 0x3b, 0x9b, 0x53, 0xc6, 0x2c, 0x18, 0x02, 0xf5, 0x1f, 0xf1,
    0xf3, 0x82, 0xdc, 0xef, 0xbf, 0x09, 0x7d, 0x97, 0xd2, 0xf6, 0xe7, 0x67, 0x51, 0x20, 0x28, 0x1c,
    0x9c, 0x2f, 0x22, 0xd6, 0x48, 0x94, 0x57, 0x4b, 0x44, 0xf8, 0x1e, 0xea, 0x6d, 0xc5, 0x8d, 0x32,
    0x3d, 0xca, 0xcc, 0xeb, 0x66, 0x7d, 0xe8, 0x3d, 0x7b,
];
...
const AES_CIPHERTEXT_WITH_NONCE: &[u8] = &[
    0x64, 0x65, 0x74, 0x65, 0x72, 0x6d, 0x69, 0x6e, 0x69, 0x73, 0x74, 0x69, 0x3a, 0x52, 0xfb, 0x62,
    0x7c, 0x15, 0x83, 0xc4, 0x6c, 0xa9, 0x0b, 0xc5, 0x5d, 0xbd, 0xa5, 0x2c, 0xc5, 0x79, 0xa9, 0x64,
    0x8c, 0xc1, 0x81, 0xa2, 0xa1, 0xbc, 0xd4, 0xb0, 0x49, 0xe3,
];
...
fn test_aes_empty_plaintext() {
    ...
    // Decrypt and verify
    let decrypted =
        decrypt_aes(None, &ciphertext, &AES_KEY, AES_AAD).expect("Should decrypt empty plaintext");
    assert_eq!(
...
fn test_aes_large_plaintext() {
    ...
    let decrypted =
        decrypt_aes(None, &ciphertext, &AES_KEY, AES_AAD).expect("Should decrypt large plaintext");
    assert_eq!(
...
fn test_aes_binary_plaintext() {
    ...
    let decrypted =
        decrypt_aes(None, &ciphertext, &AES_KEY, AES_AAD).expect("Should decrypt binary plaintext");
    assert_eq!(
