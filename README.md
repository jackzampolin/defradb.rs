...
Every release ships three FFI families: **full** (`defra-ffi_*`), with every
capability compiled in; **iroh** (`defra-ffi-iroh_*`), an iOS XCFramework
carrying the iroh transport; and **lean** (`defra-ffi-lean_*`), libp2p-only with
no Lens migrations and no SourceHub ACP. All of them expose the same `defra.h`
