...
The intended user may be on a phone, but the design may favor desktop efficiency when the improvement is material. A full node and routine download of every generation are not assumed. The current selected POC remains the baseline: replicated Dense XOR for snapshots, Compact DPF for live subscriptions, and separate OHTTP paths for origin separation.
